...
@divoc
@matchbox
Scenario: Transforming Example DIVOC to Bundle of Core Data Set VS
...
And match response.entry[0].resource.identifier == '19882120590'
And match response.entry[0].resource.certificate.issuer == 'https://cowin.gov.in/'
# And match response.entry[0].resource.certificate.version == 'unk'
And match response.entry[0].resource.certificate.hcid == '448086902'
And match response.entry[0].resource.certificate.ddccid.value == '448086902'
# And match response.entry[0].resource.certificate.period contains only {start: '', end: ''}
###### vaccination.vaccine is not mapped.
...
And match response.entry[0].resource.vaccination.validFrom == '2021-04-21'
...
@divoc
@validator
Scenario: Transforming Example DIVOC to Bundle of Core Data Set VS using validator
...
And match response.entry[0].resource.name == 'Jill C. Anyperson'
And match response.entry[0].resource.birthDate == '1985-03-04'
# And match response.entry[0].resource.identifier == '19882120590' ### NOT MAPPED ###
##### certificate.issuer fails, but there is an issuer field in the DIVOC example.
# And match response.entry[0].resource.certificate.issuer == 'https://cowin.gov.in/'
# And match response.entry[0].resource.certificate.version == 'unk'
...
Then status 200
And match response.resourceType == 'Bundle'
###### DIVOC TR structure maps not yet written.
And match response.entry == '#[1]'
And match response.entry[0].resource.resourceType == 'DDCCCoreDataSet'
And match response.entry[0].resource.name == 'Sam-21'
And match response.entry[0].resource.birthDate == '1995-10-19'
And match response.entry[0].resource.identifier == 'did:in.gov.passport:11112332334'
And match response.entry[0].resource.certificate.issuer contains {display: 'https://cowin.gov.in/'}
And match response.entry[0].resource.certificate.version == '1.3.0'
And match response.entry[0].resource.certificate.hcid contains only {value: '126'}
And match response.entry[0].resource.certificate.ddccid.value == '126'
And match response.entry[0].resource.certificate.period contains only {start: '2022-12-09T11:24:45.000+00:00', end: '2023-03-09T11:24:45.000+00:00'}
And match response.entry[0].resource.test.pathogen contains {system: 'http://id.who.int/icd11/mms', code: 'XN109'}
And match response.entry[0].resource.test.type contains only {system: 'http://worldhealthorganization.github.io/ddcc/CodeSystem/lab-test-type-divoc	', code: 'RT-PCR'}
# And match response.entry[0].resource.test.brand contains only {system: '', code: ''} ### NO SOURCE ###
And match response.entry[0].resource.test.manufacturer contains only {system: '', code: 'Bharat Biotech'}
# And match response.entry[0].resource.test.origin == 'unk' ### NO SOURCE ###
And match response.entry[0].resource.test.date == '2021-09-17T09:35:30.888Z'
And match response.entry[0].resource.test.result contains only {system: 'http://id.who.int/icd11/mms', code: 'RA01.0'}
And match response.entry[0].resource.test.centre contains only { code: 'Laboratorio ATGEN'}
And match response.entry[0].resource.test.country contains only {system: 'urn:iso:std:iso:3166', code: 'IN'}

@divoc
@validator
Scenario: Transforming Example DIVOC to Bundle of Core Data Set TR using validator
Given def response = transform('fixtures/divoc/example-divoc-test-result.json','target/divoc-valid-tr.json','http://worldhealthorganization.github.io/ddcc/StructureMap/CertDIVOCtoCoreDataSet')
Then match response.resourceType == 'Bundle'
###### DIVOC TR structure maps not yet written.
And match response.entry == '#[1]'
And match response.entry[0].resource.resourceType == 'DDCCCoreDataSet'
